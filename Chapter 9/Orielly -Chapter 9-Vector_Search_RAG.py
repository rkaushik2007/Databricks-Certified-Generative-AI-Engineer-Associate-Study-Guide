...
# MAGIC ## Scenario
# MAGIC
# MAGIC You are a **generative AI engineer** responsible for designing a high-performance retrieval-augmented generation (RAG) system that supports thousands of daily queries across your organization. Analysts depend on this system to search technical documentation, internal wikis, incident reports, and operational logs. Recent growth in usage has led to slower responses, inconsistent retrieval quality, and increased demand for scalability. Leadership has approved a platform redesign to improve retrieval accuracy, accelerate model responses, and support significantly higher traffic volumes.
# MAGIC
# MAGIC Your assignment is to build a production-ready retrieval layer using **Databricks Mosaic AI** and **Databricks Vector Search**. The lab guides you through the full workflow—from embedding generation to index creation, semantic retrieval, model serving, batching optimization, and profiling—to simulate the steps involved in deploying a scalable enterprise RAG system.
# MAGIC
# MAGIC You will perform the following tasks:
# MAGIC
# MAGIC 1. Create an embedding pipeline using a Databricks-hosted embedding model.
# MAGIC 2. Generate embeddings either through Model Serving or by using `ai_query()` for batch inference workloads.
# MAGIC 3. Build a Vector Search index and populate it with document embeddings.
# MAGIC 4. Configure a retriever to power semantic search and evaluate retrieval relevance.
# MAGIC 5. Deploy both an embedding model and a generative model through Mosaic AI Model Serving.
# MAGIC 6. Implement batching, concurrency tuning, and context-management techniques to achieve stable, low-latency inference.
# MAGIC 7. Profile end-to-end RAG performance using Databricks tools to identify latency spikes, queue wait time, inefficient batching, or overloaded index partitions.
# MAGIC 8. Apply the strategies presented in Chapter 9, including embedding-dimensionality adjustments, context-length tuning, batching improvements, and index-level configuration changes.
# MAGIC
# MAGIC This hands-on scenario mirrors real enterprise environments, where retrieval quality, scalability, and operational efficiency determine the system's usefulness. Completing this lab will help you translate the concepts introduced in Chapter 9 into practical implementation steps and prepare you for production-grade retrieval workloads.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Objective
...
# MAGIC
# MAGIC - ✅ Design and implement a scalable Databricks Vector Search index using managed or direct-access indexing patterns.
# MAGIC - ✅ Select, configure, and apply appropriate embedding models for your retrieval workload.
# MAGIC - ✅ Use Model Serving to deploy embedding models and large language models (LLMs) that support retrieval-augmented generation.
# MAGIC - ✅ Implement batching, concurrency limits, and context-length management to increase throughput and reduce inference latency.
# MAGIC - ✅ Diagnose performance issues using profiling tools that expose retrieval latency, vector-index traversal cost, model execution time, batching efficiency, and orchestration overhead.
# MAGIC - ✅ Optimize retrieval pipelines using the tuning strategies introduced in Chapter 9, including embedding-dimension selection, batch inference configuration, index rebuilding, and autoscaling adjustments.
# MAGIC - ✅ Connect observed bottlenecks to root-cause patterns such as slow similarity search, large context windows, inefficient batching, or index saturation—and apply targeted fixes.
# MAGIC
# MAGIC > **Note:** You can access this lab on GitHub here: [Chapter 9 Hands-On Lab](https://github.com/your-repo/chapter9-lab)
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Prerequisites
...
%pip install databricks-vectorsearch databricks-sdk databricks-langchain>=0.13.0 langchain>=0.3.0 langchain-text-splitters>=0.3.0 langchain-core>=0.3.0 tiktoken --quiet
...
# Databricks LangChain integration (replaces deprecated langchain_community imports)
# See: https://pypi.org/project/databricks-langchain/
from databricks_langchain import DatabricksEmbeddings, ChatDatabricks, DatabricksVectorSearch
...
# MAGIC %md
# MAGIC ### Alternative: Batch Inference with ai_query()
# MAGIC
# MAGIC For large-scale batch inference workloads, Databricks provides the `ai_query()` SQL function. This approach is ideal when:
# MAGIC
# MAGIC - Processing millions of documents in a batch job
# MAGIC - Leveraging Spark's distributed processing for parallelism
# MAGIC - Integrating embedding generation into ETL pipelines
# MAGIC
# MAGIC **ai_query() vs. Model Serving API:**
# MAGIC
# MAGIC | Approach | Best For | Latency | Throughput |
# MAGIC |----------|----------|---------|------------|
# MAGIC | **Model Serving API** | Real-time queries, interactive apps | Low (ms) | Moderate |
# MAGIC | **ai_query()** | Batch ETL, large-scale processing | Higher | Very High |
# MAGIC
# MAGIC **Example Usage (SQL):**
# MAGIC ```sql
# MAGIC SELECT
# MAGIC     chunk_id,
# MAGIC     content,
# MAGIC     ai_query('databricks-bge-large-en', content) AS embedding
# MAGIC FROM chunks_table
# MAGIC ```
# MAGIC
# MAGIC **Example Usage (PySpark):**
# MAGIC ```python
# MAGIC from pyspark.sql.functions import expr
# MAGIC
# MAGIC df_with_embeddings = df.withColumn(
# MAGIC     "embedding",
# MAGIC     expr("ai_query('databricks-bge-large-en', content)")
# MAGIC )
# MAGIC ```
# MAGIC
# MAGIC > **Note:** The `ai_query()` function requires the model to be registered in Unity Catalog or available as a Foundation Model. For this lab, we use the Model Serving API approach for interactive demonstration, but `ai_query()` is recommended for production batch workloads.
...
# MAGIC %md
# MAGIC ### Evaluate Retrieval Relevance
# MAGIC
# MAGIC Evaluating retrieval relevance is critical for understanding how well your RAG system finds the right documents. This section demonstrates key relevance metrics and evaluation techniques.
# MAGIC
# MAGIC **Why Evaluate Retrieval Relevance?**
# MAGIC - Poor retrieval = poor generation (garbage in, garbage out)
# MAGIC - Helps identify when to adjust chunk size, embedding model, or top-k
# MAGIC - Provides baseline metrics for A/B testing improvements
# MAGIC
# MAGIC **Key Relevance Metrics:**
# MAGIC
# MAGIC | Metric | Description | Interpretation |
# MAGIC |--------|-------------|----------------|
# MAGIC | **Similarity Score** | Cosine similarity (0-1) | >0.7 = strong match, <0.5 = weak match |
# MAGIC | **Precision@K** | Relevant docs in top-K / K | Higher = better ranking |
# MAGIC | **Mean Reciprocal Rank (MRR)** | 1/rank of first relevant doc | Higher = relevant docs ranked higher |
# MAGIC | **Hit Rate** | Queries with at least 1 relevant doc | Higher = better coverage |
# MAGIC
# MAGIC **What This Cell Does:**
# MAGIC 1. Defines test queries with expected relevant document titles
# MAGIC 2. Retrieves documents and calculates similarity scores
# MAGIC 3. Computes precision and hit rate metrics
# MAGIC 4. Provides actionable insights for tuning
...
def evaluate_retrieval_relevance(
    ...
    """
    Evaluate retrieval relevance using test cases with expected results.

    Args:
        test_cases: List of dicts with 'query' and 'expected_titles' keys
    ...
    for case in test_cases:
        ...
        # Calculate hit (at least one expected title found)
        hit = any(title in retrieved_titles for title in expected_titles)

        # Calculate precision (how many retrieved are relevant)
        relevant_count = sum(1 for title in retrieved_titles if title in expected_titles)
        ...
        results.append({
            ...
            "retrieved_titles": retrieved_titles[:3]  # Top 3 for display
        })
    ...
    reciprocal_ranks = [1/r["first_relevant_rank"] for r in results if r["first_relevant_rank"]]
    mrr = sum(reciprocal_ranks) / len(results) if results else 0
...
test_cases = [
    {
        "query": "How do I troubleshoot Kubernetes pod crashes?",
        "expected_titles": ["Kubernetes Deployment Troubleshooting"]
    },
    {
        "query": "What is the data retention policy for PII?",
        "expected_titles": ["Data Governance Policies"]
    },
    {
        "query": "How do I deploy a machine learning model?",
        "expected_titles": ["Machine Learning Model Deployment Guide"]
    },
    {
        "query": "What are the code review best practices?",
        "expected_titles": ["Code Review Best Practices"]
    },
    {
        "query": "What was the root cause of the payment outage?",
        "expected_titles": ["Incident Report: Payment Processing Outage"]
    }
]
...
eval_results = evaluate_retrieval_relevance(test_cases, retriever)

print(f"\n📈 Aggregate Metrics:")
print(f"   Hit Rate:           {eval_results['hit_rate']:.1%}")
print(f"   Average Precision:  {eval_results['avg_precision']:.1%}")
print(f"   Mean Reciprocal Rank (MRR): {eval_results['mrr']:.3f}")

print(f"\n📋 Detailed Results:")
print("-" * 70)
for result in eval_results["detailed_results"]:
    status = "✅" if result["hit"] else "❌"
    print(f"\n{status} Query: {result['query'][:50]}...")
    print(f"   Precision: {result['precision']:.1%} | First Relevant Rank: {result['first_relevant_rank'] or 'N/A'}")
    print(f"   Top Retrieved: {result['retrieved_titles'][0] if result['retrieved_titles'] else 'None'}")

print("\n" + "=" * 70)
print("\n💡 Interpretation Guide:")
print("   • Hit Rate > 80%: Good retrieval coverage")
print("   • Avg Precision > 40%: Reasonable ranking quality")
print("   • MRR > 0.5: Relevant docs typically in top 2 positions")
print("\n🔧 If metrics are low, consider:")
print("   • Adjusting chunk size (smaller for precision, larger for context)")
print("   • Trying different embedding models")
print("   • Increasing top-k for better recall")
...
# MAGIC
# MAGIC Based on the endpoint discovery, you have access to:
...
# MAGIC %md
# MAGIC ### Concurrency Tuning
# MAGIC
# MAGIC Concurrency tuning is essential for achieving stable, low-latency inference at scale. This section explores how different concurrency levels affect throughput and latency.
# MAGIC
# MAGIC **Concurrency Trade-offs:**
# MAGIC
# MAGIC | Workers | Throughput | Latency | Resource Usage | Best For |
# MAGIC |---------|------------|---------|----------------|----------|
# MAGIC | 1 | Low | Consistent | Minimal | Debugging, rate-limited APIs |
# MAGIC | 2-4 | Moderate | Stable | Balanced | Most production workloads |
# MAGIC | 8-16 | High | Variable | High | Burst processing, batch jobs |
# MAGIC | 16+ | Very High | Unpredictable | Very High | Only with autoscaling endpoints |
# MAGIC
# MAGIC **Key Considerations:**
# MAGIC - **Rate limits**: Model Serving endpoints have concurrency limits
# MAGIC - **Memory pressure**: Each worker holds query state in memory
# MAGIC - **Latency variance**: Higher concurrency = more latency variability
# MAGIC - **Error handling**: More workers = more potential for partial failures
# MAGIC
# MAGIC **What This Cell Does:**
# MAGIC 1. Tests different concurrency levels (1, 2, 4 workers)
# MAGIC 2. Measures throughput and latency for each configuration
# MAGIC 3. Identifies the optimal concurrency for your workload
# MAGIC 4. Provides recommendations based on results

# COMMAND ----------

# ============================================================
# CONCURRENCY TUNING EXPERIMENTS
# ============================================================

def benchmark_concurrency(queries: List[str], worker_counts: List[int]) -> Dict[str, Any]:
    ...
    for workers in worker_counts:
        ...
        start_time = time.time()
        batch_results = batch_process_queries(queries, max_workers=workers, show_progress=False)
        total_time = time.time() - start_time
        ...
        print(f"   Throughput: {results[workers]['throughput']:.2f} queries/sec")
        print(f"   Avg Latency: {results[workers]['avg_latency']:.2f}s")

    return results

# Test queries for concurrency benchmark
concurrency_test_queries = [
    "What is the authentication flow?",
    "How do I deploy a model?",
    "What are the data governance policies?",
    "How do I troubleshoot Kubernetes?",
]

print("📊 Concurrency Tuning Benchmark\n")
print("=" * 60)
...
print("\n📈 Summary:")
print("-" * 40)
print(f"{'Workers':<10} {'Throughput':<15} {'Avg Latency':<15} {'Max Latency':<15}")
print("-" * 40)
for workers, metrics in concurrency_results.items():
    print(f"{workers:<10} {metrics['throughput']:.2f} q/s{'':<6} {metrics['avg_latency']:.2f}s{'':<8} {metrics['max_latency']:.2f}s")

# Find optimal configuration
best_throughput = max(concurrency_results.items(), key=lambda x: x[1]["throughput"])
print(f"\n💡 Recommendation: Use {best_throughput[0]} workers for best throughput ({best_throughput[1]['throughput']:.2f} q/s)")
print("   Adjust based on your latency requirements and rate limits.")
...
# MAGIC %md
# MAGIC ### Advanced Profiling: Latency Spikes, Queue Wait, and Index Health
# MAGIC
# MAGIC Production RAG systems require monitoring for specific issues mentioned in Chapter 9:
# MAGIC
# MAGIC 1. **Latency Spikes** - Sudden increases in response time
# MAGIC 2. **Queue Wait Time** - Time spent waiting for model serving capacity
# MAGIC 3. **Index Partition Load** - Uneven distribution across index partitions
# MAGIC
# MAGIC **Why These Metrics Matter:**
# MAGIC
# MAGIC | Metric | Symptom | Root Cause | Fix |
# MAGIC |--------|---------|------------|-----|
# MAGIC | **Latency Spikes** | Occasional slow queries | Cold starts, GC pauses | Warm-up queries, provisioned concurrency |
# MAGIC | **Queue Wait** | Consistent delays | Insufficient endpoint capacity | Scale up, increase concurrency |
# MAGIC | **Index Saturation** | Slow search at scale | Too many vectors per partition | Rebuild index, add partitions |
# MAGIC
# MAGIC **What This Cell Does:**
# MAGIC 1. Analyzes latency distribution to detect spikes
# MAGIC 2. Simulates queue wait time measurement
# MAGIC 3. Checks index health and partition statistics
# MAGIC 4. Provides actionable diagnostics

# COMMAND ----------

# ============================================================
# ADVANCED PROFILING: LATENCY SPIKES, QUEUE WAIT, INDEX HEALTH
# ============================================================
...
print("\n📈 1. Latency Spike Analysis")
print("-" * 40)

latencies = metrics_df["total_latency"].values
p50 = np.percentile(latencies, 50)
p95 = np.percentile(latencies, 95)
p99 = np.percentile(latencies, 99)
max_latency = latencies.max()

# Detect spikes (queries > 2x P50)
spike_threshold = p50 * 2
spikes = latencies[latencies > spike_threshold]
...
print(f"   Spike Rate (>2x P50): {spike_rate:.1f}%")

if spike_rate > 10:
    print("\n   ⚠️  High spike rate detected!")
    print("   Recommendations:")
    print("   • Implement warm-up queries to avoid cold starts")
    print("   • Check for garbage collection pauses")
    print("   • Consider provisioned concurrency for consistent latency")
else:
    print("\n   ✅ Latency distribution is stable")

# 2. Queue Wait Time Estimation
print("\n📊 2. Queue Wait Time Analysis")
print("-" * 40)

# In production, queue wait would be measured from Model Serving metrics
# Here we estimate based on latency variance
latency_variance = np.var(latencies)
estimated_queue_wait = max(0, (p95 - p50) * 0.3)  # Rough estimation

print(f"   Latency Variance: {latency_variance:.4f}")
print(f"   Estimated Queue Wait: {estimated_queue_wait:.2f}s")

if estimated_queue_wait > 0.5:
    print("\n   ⚠️  Significant queue wait detected!")
    print("   Recommendations:")
    print("   • Increase Model Serving endpoint concurrency")
    print("   • Enable autoscaling for the endpoint")
    print("   • Consider request batching to reduce queue pressure")
else:
    print("\n   ✅ Queue wait is within acceptable limits")

# 3. Index Health Check
print("\n🗂️  3. Index Health and Partition Analysis")
print("-" * 40)
...
try:
    ...
    num_rows = index_status.get("num_rows", "N/A")
    ready = index_status.get("ready", False)

    print(f"   Index Status: {'Ready ✅' if ready else 'Not Ready ⚠️'}")
    print(f"   Indexed Rows: {num_rows}")

    # Check for potential index saturation
    if isinstance(num_rows, int) and num_rows > 100000:
        print("\n   💡 Large index detected. Consider:")
        print("   • Using metadata filters to reduce search scope")
        print("   • Implementing index sharding for very large datasets")
        print("   • Monitoring search latency trends over time")
    else:
...
# 4. Summary Dashboard
print("\n" + "=" * 70)
print("📋 Performance Health Summary")
print("=" * 70)
...
if spike_rate > 10:
    health_score -= 20
    issues.append("High latency spike rate")
if estimated_queue_wait > 0.5:
    health_score -= 20
    issues.append("Queue wait time")
if max_pct > 80:
    health_score -= 15
    issues.append(f"{bottleneck} bottleneck")
...
print("\n   Next Steps:")
print("   • Apply optimization strategies in Part 9")
print("   • Re-run profiling after optimizations")
print("   • Set up continuous monitoring for production")
...
# MAGIC 2. **Generated Embeddings via Model Serving and ai_query()**: Used both real-time serving and batch inference approaches
# MAGIC 3. **Built a Vector Search Index**: Set up a Delta Sync index and populated it with documents
# MAGIC 4. **Configured a Retriever and Evaluated Retrieval Relevance**: Built a LangChain retriever and measured precision, hit rate, and MRR
# MAGIC 5. **Served LLM and Embedding Models**: Integrated Mosaic AI Model Serving for generation
# MAGIC 6. **Implemented Batching, Concurrency Tuning, and Context Management**: Optimized throughput with parallel processing, worker benchmarking, and context length adjustments
# MAGIC 7. **Profiled RAG Performance**: Identified bottlenecks including latency spikes, queue wait time, and index health metrics
# MAGIC 8. **Applied Chapter 9 Optimization Strategies**: Including context-length tuning, embedding dimensionality adjustments, batching optimization, and index-level performance improvements
# MAGIC
...
# MAGIC 5. **Continuous Evaluation**: Expand relevance evaluation with more test cases and automated regression testing
# MAGIC
# MAGIC ### Resources
# MAGIC
