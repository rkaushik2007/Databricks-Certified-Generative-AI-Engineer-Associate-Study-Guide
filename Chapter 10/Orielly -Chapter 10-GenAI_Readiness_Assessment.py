...
# MAGIC - **databricks-vectorsearch** - Databricks native Vector Search client for semantic retrieval
# MAGIC - **databricks-sdk[openai]** - Databricks SDK with OpenAI-compatible client for Foundation Model APIs
# MAGIC - **tiktoken** - Token counting for context window management
# MAGIC - **faker** - For generating realistic sample data
# MAGIC
# MAGIC > **Note**: This lab uses **Databricks-native solutions** for optimal compatibility:
# MAGIC > - **Databricks Vector Search** instead of external vector databases (ChromaDB, Pinecone, etc.)
# MAGIC > - **Databricks Foundation Model APIs** via OpenAI-compatible client (recommended approach)
# MAGIC > - **Regex-based PII detection** for runtime compatibility (Presidio has numpy version conflicts)

# COMMAND ----------

# Install required packages
# Note: We use Databricks native Vector Search - no external vector DB needed
# databricks-sdk[openai] provides OpenAI-compatible client for Foundation Model APIs (recommended approach)
%pip install databricks-vectorsearch databricks-sdk[openai]>=0.35.0 tiktoken faker --quiet
...
# ML and AI imports
import mlflow
from mlflow import MlflowClient  # Updated: Use direct import (mlflow.tracking.MlflowClient is legacy)
...
# MAGIC - Uses **Databricks Foundation Model APIs** via the **OpenAI-compatible client** (recommended approach)
...
# MAGIC
# MAGIC **Modern API Approach**: We use `databricks-sdk[openai]` which provides:
# MAGIC - Automatic authentication (no manual token handling)
# MAGIC - OpenAI-compatible interface for easy migration
# MAGIC - Full support for chat completions, embeddings, and more
# MAGIC
# MAGIC Available Foundation Models in Databricks (as of 2025):
# MAGIC - `databricks-meta-llama-3-3-70b-instruct` - Meta's Llama 3.3 70B (recommended)
...
# MAGIC - `databricks-dbrx-instruct` - Databricks DBRX
# MAGIC
# MAGIC > **Note**: Check your workspace's Model Serving page for the current list of available endpoints. Model availability varies by region.
...
Please provide a helpful response based on the context above."""
    ...
    def _call_foundation_model(self, prompt: str) -> str:
        """
        Call Databricks Foundation Model API using OpenAI-compatible client.

        This uses the recommended approach with databricks-sdk[openai] which provides
        automatic authentication and OpenAI-compatible interface.
        """
        try:
            # Use the OpenAI-compatible client from Databricks SDK (recommended approach)
            from databricks.sdk import WorkspaceClient

            # Initialize the OpenAI-compatible client with automatic auth
            w = WorkspaceClient()
...
# MAGIC - Installed required libraries (Databricks Vector Search, tiktoken, faker)
